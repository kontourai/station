...
test.describe('First-run zero-provider chat rescue (#191 R1)', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
      // This journey is a returning user who already dismissed setup.
      // Clearing the flag makes the reminder legitimately win the first paint
      // and turns the absence assertion into a loading race.
  ...
  test('ends at a guided chat empty-state, never a raw error, with a one-click path to Connections -> Models', async ({
    ...
    // A returning user who dismissed setup stays in the chat recovery surface
    // when the model inventory is empty.
