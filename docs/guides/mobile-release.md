...
**TestFlight/App Store is the authoritative iOS update path.** Stable, Beta,
and Nightly TestFlight builds do not require a custom update feed; an absent
feed compiles no feed-check state and cannot prevent signing or upload. The
selected Station's Core update endpoint is intentionally not evidence for an
installed Play/TestFlight binary.

A custom feed is optional informational metadata. If none of
`VITE_NATIVE_APP_UPDATE_FEED_URL`, `VITE_NATIVE_APP_UPDATE_PROVIDER_ORIGIN`,
`NATIVE_APP_UPDATE_ACTION_URL`, `NATIVE_APP_UPDATE_ACTION_KIND`, or
`NATIVE_APP_UPDATE_ACTION_ORIGINS` is configured, the TestFlight receipt
records `TestFlight/App Store` as the update authority and `customFeed: null`.
...
configured provider origin. The `testflight-update-authority.json` provider
receipt records the optional feed metadata separately from the store update
authority.

When an optional custom feed is deliberately published, the protected publish
environment also requires `NATIVE_APP_UPDATE_PUBLISH_TOKEN`. Station supports
an HTTPS provider that atomically replaces the exact feed resource with
conditional `PUT` (`If-Match`/`If-None-Match`). After the GitHub draft is
...
redirects, or a mismatched response fail that configured feed publication. The
action URL must be an HTTPS page where the published app can actually be
downloaded or installed. `NATIVE_APP_UPDATE_ACTION_KIND` must be `artifact` or
`store-page`; publication verifies the resolved response is respectively a
downloadable package response or an HTML installation page before mutating the
feed.
