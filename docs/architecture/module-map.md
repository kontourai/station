...
| [PluginCommandEffects](#plugincommandeffects) | Admit one plugin palette effect per browser document and report withdrawals honestly until each captured effect settles. | `src-server/services/plugins/plugin-command-effects.ts` |
...
**Intent and Interface.** A plugin command row in the palette is not authority (kontourai/station#1418). `createPluginCommandEffectAdmission().admit()` admits one argument-free `navigate` or `seed-composer` effect for one browser document and returns a receipt whose effect content Station read from the installed declaration. `PluginCommandEffectService` owns the durable ledger in the Station home (`plugin-command-effects.json`): `recordAdmission`, `settle`, `beginWithdrawal`, `withdrawal`, `awaitWithdrawal` and `resolveWithdrawal`. Wire shapes live in `@kontourai/station-contracts/plugin-command-effect`; admissions and settlements are also recorded as `station.plugin-command.execution/v1` operational events.

**Contract.** Linearization points (kontourai/station#1419): LP-A is the atomic ledger append of an `admitted` effect, reached only after visibility (an invisible plugin is refused as absent), then inside the plugin content lock the installed artifact, exact generation, declaration, target and awaited requirements, a fresh currentness check, and for `plugin-server` commands the grants read lease. LP-W is `beginWithdrawal`, called after an authority change is durable and under the serialization admission of that authority uses: uninstall and install-over inside the install transaction's content lock, the legacy update route inside its content lock, and `plugin.server` withdrawal after the grants write that admissions append inside. LP-K is the atomic settlement write: first terminal outcome wins, the same outcome is idempotent, a different one is a counted 409 conflict, and a cancel before any admission is recorded so that admission refuses. LP-C is a ledger read finding every captured effect settled with document or station proof. A withdrawal is `completed` only then; it is `winding-down` while young, `indeterminate` after the wait, and `closed-indeterminate` only by operator decision, which abandons the outstanding effects and still counts late settlements. Capacity (8 outstanding per plugin, 64 in total, 64 open withdrawals) refuses new work and never evicts an outstanding effect or an open withdrawal. A withdrawal is recorded only when it captured something. Lifecycle responses wait briefly after releasing their locks and answer 202 with `commandEffects` while an effect is outstanding. The ledger lock is always taken last.

**Seam, Implementation, callers, and tests.** `routes/plugins/plugin-command-effect-routes.ts` is the HTTP seam; `plugins.ts` composes it with principal resolution, plugin visibility, the runtime audit publisher and requirement resolution, and hosted deployments refuse it. `plugin-install-transaction.ts`, `plugin-lifecycle-routes.ts`, `plugin-public-routes.ts` and `plugin-host-approval-routes.ts` call `withdrawPluginCommandEffects`; lifecycle routes call `settlePluginCommandEffectsForResponse`. Ledger invariants, bounds, fault-injected commits and restart are in `plugin-command-effects.test.ts`; the four real withdrawal paths against forced interleavings are in `plugin-command-effect-lifecycle.test.ts`. **Do not reintroduce:** a palette row or cached intent as effect content, an audit event as the admission record, eviction of outstanding effects, a withdrawal reported complete without settlement proof, or a lifecycle response that waits while holding a plugin lock.
