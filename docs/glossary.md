...
- **Draft** (session state) — a Session that exists but that nothing has been sent to: no turn has started anywhere in its conversation's lineage, and it carries no history from elsewhere (attached, adopted, or a Station-dispatched delegation). The server derives it (`OrchestrationSessionSummary.draft`, #2310) so every device agrees; the first `turn.started` ends it. A Draft is listed under **Drafts**, never under "Active now". Not the same thing as a composer draft — unsent text in a chat's input, kept per device.
