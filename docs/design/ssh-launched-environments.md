...
3. Before installation, the launcher probes `http://127.0.0.1:<remote-port>/.well-known/station/v1` over SSH. A live server is reused and that fact is recorded. Otherwise it runs `npm ci`, then starts `STATION_HOST=127.0.0.1 ./station start --instance=ssh-launched --port=<remote-port>` from the verified checkout. Because checkout, install, and build can be slow, Tauri returns a launch id immediately; the UI polls status through `probing`, `cloning`, `installing`, `starting`, and `ready` or `failed`. Failure includes the failing step's bounded stderr tail.
