...
- **Registration.** `POST /api/system/native-push/register` from a paired
  ...
  delivers to, and the route sits on the `/api/system` operate tier.
  `DELETE /api/system/native-push` clears the
