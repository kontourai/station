...
*Version note (2026-07-07, station#218): Station now pins exact `@kontourai/flow-agents@3.3.0` (no caret). The `legacyFlowAgentsArtifactRoot`/`LEGACY_FLOW_AGENTS_DIR` exports named above were removed upstream in 3.x (`.flow-agents` is now exclusively the package's own durable install/config root, `DURABLE_FLOW_AGENTS_DIR` — a different concept); Station now implements that legacy-compat helper locally in `local-artifact-paths.ts` instead of importing it. Full subcommand-by-subcommand CLI behavior audit (2.2.0 -> 3.3.0): `.kontourai/flow-agents/s218-flow-agents-3x/cli-audit-2.2.0-to-3.3.0.md`.*
