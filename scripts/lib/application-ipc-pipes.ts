...
export function applicationIpcPipes(input:Writable,output:Readable,accept:(channel:ApplicationChannel)=>void){const ipc=new PionApplicationIpc(input,output,accept);return{prepareClose:()=>ipc.close(),finish:()=>ipc.finish()};}
