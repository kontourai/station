...
function benignForRepoDocs(code, value) {
  ...
  if (code === 'private-ip') {
    ...
    // is a prefix with no groups after it, not an address. The pattern itself
    // now rejects a colonless `fd…` run (a git SHA), so only this trailing-
    // colon shape still reaches here.
