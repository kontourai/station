 ...
 * (spawnLongRunningFixtureChild) in a genuinely separate process via
 * abnormal-exit-harness.ts, sends that process SIGTERM before it has any
 * chance to run an afterEach, and asserts the grandchild it spawned did not
 * survive. Independent proof of the scripts/ implementation -- it is a
 * deliberate duplicate of packages/cli/src/__tests__/helpers/longrunning-fixture-child.ts
 * (see that file for why), so passing there does not prove this one.
 */
...
describe('abnormal suite teardown (station#1812)', () => {
  it('reaps the fixture grandchild even when the spawning worker is SIGTERMed before any afterEach runs', async () => {
    ...
    // repo (Node's own native TS support requires literal `.ts`
    // specifiers, which this repo's tsc config rejects without an extra
    // compiler flag it doesn't otherwise need).
    ...
    // Simulate an external kill of the whole worker process.
