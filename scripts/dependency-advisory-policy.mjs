...
export async function runPolicyCli() {
  ...
  // Say why on this path too. Logging only the skip made a classification
  // fallback indistinguishable from a real dependency change for anyone
  // reading a failed run, which is how #1430 survived: every pull request was
  // reaching the registry and the log gave no hint that was unintended.
  console.log(
    `Dependency advisory floor: live registry scan required (${decision.reason})`,
  );
  const scopes = [
