...
impl SharedPreferencesEditor {
    ...
    pub fn commit(&self, env: &mut JNIEnv) -> AndroidKeyringResult<()> {
        struct ThisMethod;
        impl Method for ThisMethod {
            type Param = NoParam;
            type Return = bool;
        ...
        if ThisMethod::call(&self.self_, env, NoParam)? {
            Ok(())
        } else {
            Err(AndroidKeyringError::SharedPreferencesCommitRejected)
        }
    }
}
