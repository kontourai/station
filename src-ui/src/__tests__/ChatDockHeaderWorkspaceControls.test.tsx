...
function renderHeader(
  ...
  return { ...render(<ChatDockHeader {...props} />), props };
}
...
describe('header inbox toggle (from #1064 AC1/AC2)', () => {
  test('open state renders "Collapse chat list" with aria-pressed=true', () => {
    renderHeader({ workspaceControls: workspaceControls() });

    const toggle = screen.getByRole('button', { name: 'Collapse chat list' });
  ...
  test('collapsed state renders "Expand chat list" with aria-pressed=false', () => {
    renderHeader({
      workspaceControls: workspaceControls({ isInboxOpen: false }),
    });

    const toggle = screen.getByRole('button', { name: 'Expand chat list' });
...
describe('header background tasks button (from #1064 AC3)', () => {
  test('keeps its label, aria-haspopup/aria-expanded, and the shared shell class', () => {
    renderHeader({
  ...
  test('shows the running-count variant label and badge when tasks are running', () => {
    renderHeader({
    ...
    expect(
      screen.getByRole('button', { name: 'Background tasks — 3 running' }),
    ).toBeTruthy();
...
describe('one-bar rule (#3309)', () => {
  test('Open/New live in the header while the pane is open', () => {
