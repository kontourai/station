...
// station#4525: `DockShell` (via `useDockShellChrome`) now reads
...
describe('ChatDock activity region', () => {
  test('keeps the named dock root, every passive reset modality, and shortcut focus wiring together', () => {
    ...
    // occupant holds it (station#3929), so it reads navigation. Mounting the
    ...
    const { container } = render(
      // `DockShell` (station#4460) registers `dock.toggle`/`dock.maximize`
    ...
    // The real ambient host stays chromeless, so `DockShell` (station#4460)
 ...
 * regex can; review finding MED-4). Used only for the one property below
 * that a pure function genuinely cannot carry (station#4525 review LOW-1:
...
/**
 * station#4525/#4524 review (round 2): `AmbientChatDockProjectBinding.test.tsx`
 ...
 * (station#4525 review LOW-1: a fork confirmation must never sync the
...
describe('ChatDock project-binding wiring (station#4525/#4524, minimal call-site pins)', () => {
  test('the badge project name is wired from resolveDockBadgeProjectName, not a literal or the raw session', () => {
    ...
    // every rendered-name assertion stays green (delta review LOW-A).
  ...
  test('session facts (directory/git/coding-layout) are never gated on the badge (station#4525 review HIGH-2)', () => {
    ...
    // filter, exactly as pre-station#4525) rather than merely excluding one
    ...
    // The pre-fix-reintroduction shape review HIGH-2 actually caught, kept
  ...
  test('handleSwitchProject rebinds the shell and never opens the New Chat modal (station#4524)', () => {
    ...
    // The pre-fix behavior (#4524's reported bug): the row action opened
  ...
  // station#4525 review LOW-1: a fork is none of the three things the
