...
/**
 * Client-side translation layer for chat/provider-connection failures — #191
 * R2. Turns raw, developer-shaped error text (an HTTP status + a server or
 ...
 * (`ChatEmptyState.tsx` / `onboardingGateUtils.ts`, #191 R1) — it exists for
 ...
 * Stop-short risks). S7 (#196) is where the live text gets confirmed against
...
export interface ChatErrorTranslation {
  ...
  hint?: string;
  /**
   * station#1827: true when this classification's underlying failure is a
  ...
  /**
   * station#1827: true when `formatChatErrorDisplay` should append the raw
...
export interface ChatErrorInput {
  ...
  message: string;
  /**
   * station#1827: the originating `RuntimeErrorEvent.code`, when the caller
...
/**
 * station#1207: matches BOTH stall-watchdog shapes, not just the direct
 * chat path — review round 2 caught that the original pattern
 ...
 * instance (a stale credential, #3297) it would not have.
 */
...
/**
 * station#3299, banner-side: returns product copy ONLY when the text's
...
/**
 * station#1827 fallback net: only consulted when no `code` was supplied
...
/**
 * station#3120: extracts the observed busy percentage FROM the server's own
 ...
 * never gave it is the same defect as inventing it (review of station#3120).
 */
 ...
 *
 * Classification order (first match wins). station#3120: every structured
 ...
 * patterns collide."
 *   -3. `code === PRINCIPAL_UNRESOLVED_CODE` (station#4518) -> the request's
 ...
 *       unsafe to resume.
 *   -1. `code` identifies a terminal engine binding (station#1827) -> this
 ...
 *   0a. `code === 'resource_posture_critical'` (station#3089) -> the host
 ...
 *      mid-turn (station#1207) — a dropped/crashed transport, not a stopped
 *      response.
 ...
 *   7. Stream-ended-without-a-parseable-body-shaped message (station#3299)
 ...
 *      assertion (station#3299: "Retry your request." was such an
...
export function translateChatError(
  ...
  // station#4518: the request's caller could not be resolved to a principal
  ...
  if (code === PRINCIPAL_UNRESOLVED_CODE) {
    // MED-1 (station#4518 fix round): the body must be a short, human CANNED
    ...
    // #3120-class defect this file documents elsewhere (carrying an internal
    ...
    //
    // LOW-B (fix round, delta review): deliberately NO `disclosureRaw` here.
  ...
  // station#3089/#3120: a structured, backend-authoritative refusal from
  ...
  // busyPercent=<N>, thresholdPercent=<M>, ...`). #3120: showing that raw
  ...
  // via `disclosureRaw` (the existing station#1827 mechanism: a de-emphasized
  ...
  // station#3299: see STREAM_BODY_ENDED_PATTERN. Placed after every
  ...
  return {
    ...
    // station#3299: the fallback classified nothing, so it must not assert
 ...
 * (station#1827): the engine's own text, appended as a clearly-labeled,
