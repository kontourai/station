...
/**
 * UX audit V3 (live-reproduced): the chat dock's failure banner reads the
...
/**
 * station#1225 review (MEDIUM fix): `queryClient`, when supplied by the
...
export function ensureOrchestrationEventStream(
  ...
  // station#1092: dedup guard against duplicate/overlapping frames on a
  // sequence-cursor resume (AC3). Applying a stale duplicate here would
  ...
  // independent dedup of its own. Safe unconditionally: a pre-#1092 host
  // never sets a frame `id:`, so the guard never drops anything against it.
  const cursor = createStreamCursorTracker();
  // station#1225: the FIRST snapshot this stream instance ever receives is
  ...
  const authenticatedStream = fetchSSE(`${apiBase}/api/orchestration/events`, {
    ...
    // station#1848: a ceiling equal to the initial delay is not a backoff
    ...
    onMessage: (raw) => {
      if (raw.event === 'orchestration:snapshot') {
        ...
        // station#1410: the frame is a wrapper, not a bare event — the
        ...
        // station#4054: this frame carries no claim beyond "re-read the
    ...
    onError: () => {},
    // station#1094 review (HIGH): a TERMINAL (401/403) failure now parks
