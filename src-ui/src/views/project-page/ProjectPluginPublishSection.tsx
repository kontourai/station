...
/**
 * "Publish to git" for a Project whose folder is a plugin (#2374, epic
 * #2323 S6).
 *
 * On mount it asks only the summary, which reads plugin.json and runs no
 * git. The full inspection (the files that would be published, what is
 * skipped, secrets) is asked when the operator opens the dialog; it walks
 * the folder and never runs git in it. The route is operator-only, so a
 * collaborator, or a Project that is not a plugin, sees nothing here.
 */
  ...
  return (
    ...
    >
      ...
      <p className="project-page__section-explainer">
        Publish this plugin's files to a git repository. Its address then
        installs the plugin on any Station.
      </p>
