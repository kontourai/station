    ...
    const providerTasks: ChatBackgroundTask[] = [
      {
        ...
        // #2457: a live status line, which becomes the card's detail.
...
const EXPECTED: Record<string, string> = {
  ...
  // #2530 review 2 round 2 (DELIBERATE change to the frozen string): del-4
  // idles with no terminal lifecycle fact (no completed/failed/cancelled),
  // so its snapshot demotion is 'unresolved' now, not 'stopped' \u2014 'stopped'
  // claimed a stop nothing derived. del-5 vanishes from the snapshot
  // entirely instead of idling, so it still hits the separate stale-liveness
  // sweep below, which is unchanged and still reads 'stopped' there.
  snapshotSeedDemoteStale:
    '{"entries":{"call-1":{"id":"call-1","kind":"tool","source":"tool-event","chatThreadId":"chat-1","title":"Bash","startedAt":1790157540000,"state":"stopped","endedAt":1790164800000},"del-4":{"id":"del-4","kind":"agent","source":"delegate-session","chatThreadId":"chat-1","delegateThreadId":"del-4","stop":{"kind":"delegate-interrupt"},"title":"Delegated task \u2014 reviewer","startedAt":1790155800000,"state":"unresolved","endedAt":1790155860000},"del-5":{"id":"del-5","kind":"agent","source":"delegate-session","chatThreadId":"chat-1","delegateThreadId":"del-5","stop":{"kind":"delegate-interrupt"},"title":"Delegated task","startedAt":1790156400000,"state":"stopped","endedAt":1790164800000}},"delegateParents":{"del-4":"chat-1","del-5":"chat-1"}}',
  pruneFinished:
  ...
  // #2459 R2: task-b and task-c have no spawning tool card, so they carry no
  // start (it was the clock's `Date.now()`, an invented value).
  // #2457 (DELIBERATE change to the frozen string): task-a reports a progress
  // line, and a provider card's detail is that line when present — it was
  // always the subagent type. task-b (no progress) keeps its type as detail.
  providerSelection:
