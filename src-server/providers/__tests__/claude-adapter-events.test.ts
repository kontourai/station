...
/**
 * #2309 (owner decision): unprompted engine output is a real turn. These
 * replay the observed shape — the CLI finishing background work, then
 * replying on its own with no dispatched turn — through the real mapper.
 */
describe('claude provider-triggered turns (#2309)', () => {
  const IDLE_THREAD = 'thread-provider-turn';

  function idleRecord(): ClaudeMessageState {
    return {
      session: {
        provider: 'claude' as const,
  ...
  function map(
    ...
    mapClaudeSdkMessage({
      ...
      publish,
      ...(logInfo ? { logInfo } : {}),
      message: { session_id: IDLE_THREAD, uuid: 'u', ...message } as never,
    });
  }
  ...
  const topLevelStart = {
    type: 'stream_event',
    parent_tool_use_id: null,
    event: { type: 'message_start', message: { id: 'msg_unprompted' } },
  };
  const topLevelDelta = (text: string) => ({
    type: 'stream_event',
    parent_tool_use_id: null,
    event: {
      type: 'content_block_delta',
  ...
  const successResult = {
    ...
    is_error: false,
    num_turns: 1,
    result: 'done on my own',
    stop_reason: 'end_turn',
    usage: { input_tokens: 4, output_tokens: 2 },
  };

  test('an unprompted top-level reply is published as a provider turn and closed by its own result', () => {
    const events: CanonicalRuntimeEvent[] = [];
    const logInfo = vi.fn();
    const record = idleRecord();
    map(record, (event) => events.push(event), topLevelStart, logInfo);
    map(record, (event) => events.push(event), topLevelDelta('hi'), logInfo);
    map(record, (event) => events.push(event), successResult, logInfo);
    ...
    const started = events[0] as Extract<
      ...
      { method: 'turn.started' }
    >;
    expect(started.turnId).toMatch(/^provider:/);
    expect(started.metadata).toEqual({ trigger: 'provider' });
    expect(started.prompt).toBeUndefined();
    // Every frame of the reply, and its end, name that one turn.
    for (const event of events.slice(1))
      expect(event.turnId).toBe(started.turnId);
    expect(events[1]?.itemId as string).toContain(`:${started.turnId}:`);
    expect(
      (events[3] as { metadata?: Record<string, unknown> }).metadata,
    ).toMatchObject({ trigger: 'provider' });
  ...
  test('background subagent chatter between turns opens no turn', () => {
    const events: CanonicalRuntimeEvent[] = [];
    const record = idleRecord();
    const push = (event: CanonicalRuntimeEvent) => events.push(event);
    map(record, push, {
      type: 'system',
      subtype: 'task_started',
      task_id: 'task-bg',
    ...
    map(record, push, {
      ...
      subtype: 'task_progress',
      task_id: 'task-bg',
      description: 'Reading translateMuseRecord',
      last_tool_name: 'Read',
    });
    map(record, push, {
      type: 'tool_progress',
    ...
    // The subagent's own model stream carries its parent tool id.
    map(record, push, {
      ...topLevelStart,
      parent_tool_use_id: 'toolu-bg',
    });
    map(record, push, {
      ...topLevelDelta('subagent text'),
      parent_tool_use_id: 'toolu-bg',
    });
    // A ping is not the start of a reply.
    map(record, push, {
      type: 'stream_event',
      parent_tool_use_id: null,
      event: { type: 'ping' },
    ...
    expect(events.map((event) => event.method)).not.toContain('turn.started');
    expect(record.providerTurnId).toBeUndefined();
    expect(activeTurnIdForEvents(events)).toBeUndefined();
  });

  test('a send queued behind a provider turn is not completed by the provider reply', () => {
    const events: CanonicalRuntimeEvent[] = [];
    const push = (event: CanonicalRuntimeEvent) => events.push(event);
    const record = idleRecord();
    map(record, push, topLevelStart);
    const providerTurnId = record.providerTurnId!;
    // sendTurn runs while the provider reply is still streaming: it
    // allocates, queues and dispatches its own turn.
    record.activeTurnId = 'user-turn';
    record.dispatchedTurnId = 'user-turn';
    push({
      eventId: 'user-started',
      provider: 'claude',
      threadId: IDLE_THREAD,
      createdAt: '2026-09-22T19:00:05.000Z',
      turnId: 'user-turn',
      method: 'turn.started',
      prompt: 'follow-up',
    });

    // The SDK answers in order: the provider reply's result comes first.
    map(record, push, successResult);
    const completions = events.filter(
      (event) => event.method === 'turn.completed',
    );
    expect(completions.map((event) => event.turnId)).toEqual([providerTurnId]);
    expect(record.activeTurnId).toBe('user-turn');
    expect(record.dispatchedTurnId).toBe('user-turn');
    // The fold reads the queued user turn as the one running now.
    expect(activeTurnIdForEvents(events)).toBe('user-turn');

    map(record, push, { ...successResult, result: 'answered the user' });
    expect(
      events
        .filter((event) => event.method === 'turn.completed')
        .map((event) => event.turnId),
    ).toEqual([providerTurnId, 'user-turn']);
    expect(activeTurnIdForEvents(events)).toBeUndefined();
  });

  test('a sendTurn that allocated but had not dispatched when the reply began gets its id back', () => {
    const events: CanonicalRuntimeEvent[] = [];
    const push = (event: CanonicalRuntimeEvent) => events.push(event);
    const record = idleRecord();
    record.activeTurnId = 'allocating-turn';
    map(record, push, topLevelStart);
    expect(record.activeTurnId).toBe(record.providerTurnId);
    // Its prompt enters the queue while the provider reply streams.
    record.dispatchedTurnId = 'allocating-turn';
    map(record, push, successResult);
    expect(record.providerTurnId).toBeUndefined();
    expect(record.activeTurnId).toBe('allocating-turn');
  });

  test('a Stop of a provider turn consumes its interrupted result without a runtime error', () => {
    const events: CanonicalRuntimeEvent[] = [];
    const push = (event: CanonicalRuntimeEvent) => events.push(event);
    const record = idleRecord();
    map(record, push, topLevelStart);
    const providerTurnId = record.providerTurnId!;
    // `interruptTurn` targets `activeTurnId`, which names the provider turn.
    expect(record.activeTurnId).toBe(providerTurnId);
    record.interruptingTurnId = providerTurnId;
    map(record, push, {
      type: 'result',
      subtype: 'error_during_execution',
      is_error: true,
    ...
    expect(events.map((event) => event.method)).not.toContain('runtime.error');
    expect(record.providerTurnId).toBeUndefined();
    expect(record.activeTurnId).toBeUndefined();
  });
});
