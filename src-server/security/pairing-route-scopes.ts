/**
 * Scoped pairing (station#1098): the single route -> required-scope table.
 *
 ...
 *
 * The scope vocabulary is deliberately small — four scopes at station#1098,
 * a fifth (`inference:invoke`) added by station#1398 slice 2 (see
 ...
 *
 * station#1131 closed the coverage gap that made the endpoint above a
...
export interface PairingScopeRouteRule {
  ...
  /**
   * station#1131: `'family'` for a rule generated by the generic
...
const PAIRING_SCOPE_DOMAIN_PREFIXES: readonly string[] = [
  ...
  '/api/spatial-board',
  '/api/orchestration',
  // station#3677 PR 3: the native consent broker. The FAMILY sits on the
  ...
  // prefix above). station#1131's leaf-level scan found `/tool-approval`
  ...
  // resolved `undefined`. station#2051 requires ordinary protected callers,
...
export const PAIRING_SCOPE_ROUTE_TABLE: readonly PairingScopeRouteRule[] = [
  ...
  // `access:manage`.
  //
  // Be precise about who that is, because station#1887 documents that this
  ...
  // Pairing/device management (station#1098's access:manage) — every method,
  ...
  // station#1097 review round 2 (HIGH), owner decision "tighten now, loosen
  ...
  // station#3385: the attachment blob route is a single GET leaf, declared
  ...
  // station#4079: the board face (createBoardRoutes, routes/board.ts).
  ...
  //
  // station#4181 raised the three mutation leaves to `orchestration:operate`
  ...
  // mutations (station#4079's honest declaration of `authorizeReference`'s
  ...
  // station#1398 slice 4: the two fleet receipt leaves under `/monitoring`
  ...
  //
  // Raised to `access:manage`, not `orchestration:operate` (station#1398
  ...
  // station#1423: the operator's answer-share management family. Every
  ...
  // the higher gate on the mutation decorative (station#1398 security review,
  // M-4).
  //
  ...
  // station#2037: an exact unattended-tool grant changes what a principal may
  ...
  // station#1131 review round 1 (HIGH, own-audit finding beyond what the
  ...
  // station#3677 PR 2 review (BLOCKING): the Home role request family is the
  ...
  // station#1123 slice 2: `/api/environments/peers/**` reads and writes the
  ...
  // remote device — however broadly scoped — should ever reach. station#2051
  ...
  // station#3552: per-account quota. Deliberately the SAME tier as
  ...
  // station#3549: enrolment. Same tier as its two neighbours above, and for a
  ...
  // station#1398 slice 2 (docs/design/inference-fleet.md §3.3): the fleet
  ...
  // station#1398 §10 OQ-2 / §5.3: raise `GET /api/connections/model-inventory`
  ...
  // enumerates every model this Station can launch, #1398 turns that list
 ...
 * (station#3677) — the two `EnvironmentSecurityService` methods it needs,
 ...
 * credential (owner decision 1, station#3677):
 *
...
/**
 * station#2000: the one authoritative classification for every externally
...
export const EXTERNAL_SURFACE_CAPABILITY_TABLE: readonly ExternalSurfaceCapabilityRule[] =
  [
    ...
    // station#3677: the distinct-origin consent listener. A separate port,
...
/**
 * station#3677: the consent listener's own coverage guard. The listener is a
 ...
 * scope) so the leaf-coverage guard (station#1131) can tell whether the
...
/**
 * station#1131: every leaf `pairing-route-leaf-scan.ts` discovers under an
 ...
 * existed then (see PR for station#1131's audit notes on what was checked
...
export const PAIRING_SCOPE_FAMILY_INHERITED_LEAVES: readonly PairingScopeFamilyInheritedLeaf[] =
  [
    ...
    // station#4075 stage 3 slice 2: the session-agnostic presence roster.
    ...
    // station#2463 (fork-and-continue) shipped this leaf without declaring it,
    ...
    // connection/engine id, station#977's CLI classification lookup): no
    // other environment's or Station's data, no mutation — strictly less
    // sensitive than the sibling reads inherited here. (Merge-gap
    // fix-forward: #1142's leaf gate and station#977's route each landed
    ...
    // station#1896 logging slice 2: server log entries are redacted on
    ...
    // station#2802: a thread's recorded turn-checkpoint outcomes. Deliberate
    ...
    // station#1502 slice 4 — the resolution surface. BOTH of its leaves take
    ...
    // answer (#1392's membership model is the obvious way), that is the point
    ...
    // station#3802: added by #3798 without a scope decision, which left the
    ...
    // station#1502 slice 4 — see the `POST /api/projects/:slug/bind` note
    ...
    // station#3089: a read of this Station's own currently observed CPU
    ...
    // station#1131 review round 1 (HIGH): `createPluginRoutes`
    ...
    // station#3677 PR 3: locality-gated in-handler (see the family comment
    ...
    // station#3122 stage 3 / #3677 PR 2: the Home role's leaves. GET reads a
    ...
    // station#3673). The GRANT channel now exists: request creation and the
    ...
    // DELETE /api/plugins/:name/grant (station#3815) WITHDRAWS permissions.
...
/**
 * Leaf-level explicit-declaration check (station#1131): true when `method`
