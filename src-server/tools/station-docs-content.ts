...
export const STATION_DOCS_TOPICS: readonly StationDocsTopic[] = [
  ...
  },
  {
    ...
    body: [
      'Station ships one default, non-deletable agent whose job is to operate Station itself: create agents, run jobs, change settings, check plugins it helped write, and generally reshape the workspace the way the UI does. It cannot install a plugin: an install is approved by a person, on a preview, in the Plugins view or the `station` CLI.',
  ...
  },
  {
    ...
    body: [
      ...
      [
        ...
        '  plugin.json',
        '  src/index.tsx',
        '  src/pulse.css      (optional; import it from index.tsx)',
        '  package.json       (optional; see "Build" below)',
      ].join('\n'),
      'THE MANIFEST. `plugin.json` is an Agent Plugins 1.0 document. The root holds only portable fields: `$schema`, `name`, `version`, `description` (and optionally `author`, `homepage`, `repository`, `license`, `keywords`). Everything Station-specific lives under `extensions["io.kontourai.station"]`: `schemaVersion` (always "1.0"), `title` (the display name), `entrypoint` (path to the React module, relative to the plugin root), `capabilities` (descriptive tags such as "chat" or "navigation"), `permissions` (what the plugin asks a person to grant) and `workspacePanes` (the panes it adds). Station fields placed at the root are ignored with a warning, and a root `layout` or `layouts` makes the manifest invalid.',
      [
        'A complete minimal manifest, one pane that needs a Project:',
        '{',
      ...
      'NAMES AND IDS. `name` is the plugin id: 1-64 lowercase letters, digits, hyphens or periods, starting and ending with a letter or digit, with no "--" or "..". Capability and permission entries are lowercase too; Station disables the whole extension when one has an uppercase letter or a space. Pane ids and renderer ids are opaque strings, but they are global across every installed plugin: follow the `pane:plugin%3A<plugin-name>:<group>:<name>` and `renderer:plugin%3A<plugin-name>:<renderer kind>:<name>` pattern above so yours cannot collide, write the parts you choose in lowercase, and give every pane its own `id` and its own `rendererId`. `provenance` must be `{ "origin": "plugin", "pluginId": "<your plugin name>" }` exactly.',
      'WORKSPACE PANE FIELDS. `version` is "1.0". `name` is what a person sees in Add pane. `placement.supportedRegions` lists where the pane may go: "primary", "secondary", "standalone" or "docked"; `preferredRegion`, if given, must be one of them. `modes` needs at least one entry; "default" with `contextRequirement: { "project": true }` means the pane is offered inside a Project. Other requirement keys are "task", "session", "run", "workspace" and "source", each `true` when required. `lifecycle.stage` is "stable", "preview" or "deprecated". Optional fields: `description`, `icon`, and `order` inside `placement`. The retired top-level `contextRequirement` and `dockability` fields are refused; put the requirement inside a mode.',
      'RENDERER KINDS. Use "plugin-component" when the pane is your own React UI: `renderer.name` must be a key of the `components` object your entrypoint exports, and the component runs in Station\'s page with the SDK available. Use "mcp-tool-ui" when the UI is served by a tool on an MCP server Station knows: the renderer is `{ "kind": "mcp-tool-ui", "ref": "<serverId>/<toolName>" }`, Station renders that tool\'s UI resource in a sandboxed frame, and no entrypoint is needed. Prefer "plugin-component" for panes that read Station data through SDK hooks; prefer "mcp-tool-ui" when the data and the UI already live behind an MCP server.',
      [
      ...
      'PERMISSIONS. Declare only what the plugin uses. "navigation.dock" is granted automatically. "network.fetch", "agents.invoke", "tools.invoke" and "ui.confirm" need a person\'s consent when they install. "providers.register", "system.config", "plugin.server", "events.subscribe" and "events.read-payload" need a separate host approval. A permission Station does not recognise is treated as the strictest tier.',
      ...
      "VALIDATE BEFORE ASKING FOR AN INSTALL. If you have `station-control`, call `validate_plugin` with the plugin folder's absolute path (or an HTTPS git URL). It runs the same checks an install preview runs, and reports every problem as a diagnostic: manifest errors, an io.kontourai.station extension Station would disable (which silently drops every pane), unsafe prompt files, pane ids already taken by another plugin, and a missing entrypoint. `valid: true` means no error-level diagnostic. It does not build the bundle, so a TypeScript or import error only shows at install; typecheck locally first. Fix every error and validate again before handing the plugin over.",
      "INSTALL IS A PERSON'S DECISION. An agent cannot install a plugin, and `install_plugin` will refuse. A person installs it from Plugins → Install plugin, entering the folder path or git URL, or runs `station plugin install <path-or-url>`. Station shows a preview of what the plugin contributes and the permissions it asks for, and installs only after the person consents. After installing, the person adds the pane to a Project with Add pane. When you finish a plugin, tell the person the folder path and exactly those steps.",
      [
        'COMMON MISTAKES:',
        ...
        '- A pane whose `provenance.pluginId` is not the manifest `name`.',
