...
export const STATION_DOCS_TOPICS: readonly StationDocsTopic[] = [
  ...
  },
  {
    ...
    body: [
      ...
      'VALIDATE BEFORE ASKING FOR AN INSTALL. If you have `station-control`, call `validate_plugin` with the absolute path of the plugin folder. It checks local folders only; it does not fetch git sources. It runs part of what the install preview checks and reports each problem as a diagnostic: manifest errors, an io.kontourai.station extension Station would disable (which silently drops every pane), unsafe prompt files, pane ids already taken by another plugin, and a missing entrypoint. It does not resolve plugin dependencies (it warns `dependencies-not-checked` when you declare any) and it does not build the bundle, so a TypeScript or import error only shows at install; typecheck locally first. `valid: true` means no error-level diagnostic, not a guaranteed install. Fix every error and validate again before handing the plugin over.',
