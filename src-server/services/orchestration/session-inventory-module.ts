...
function gap(
  ...
  kind: 'not-captured' | 'unsupported-version' = 'not-captured',
  code?:
...
function empty(id: SessionInventoryGroupId): SessionInventoryGroup {
  return {
    id,
...
export function createSessionInventoryModule(input: {
  ...
  const folded = async (
    ...
    groups: Partial<Record<SessionInventoryGroupId, SessionInventoryGroup>>;
    highWater: number;
    continuation?: { sequence: number; eventId: string };
  }> => {
    ...
    const ownerGroup = (
      id: 'inputs' | 'execution' | 'decisions' | 'resources',
      items: SessionInventoryGroup['items'],
    ): SessionInventoryGroup => ({
      id,
      owner: owner('thread.canonical-events'),
      state: items.length ? 'available' : 'empty',
  ...
  const digest = (items: readonly SessionInventoryRow[]) =>
  ...
  const outputs = async (
    ...
    page = false,
    cursor?: string,
  ): Promise<SessionInventoryGroup | undefined> => {
  ...
  return {
    async read({ scope, authority, current, keptRows }) {
      ...
      ) as Partial<Record<SessionInventoryGroupId, SessionInventoryGroup>>;
    ...
    async page({ scope, groupId, continuation, authority, current, keptRows }) {
      if (!canRead(scope.sessionId, authority, current))
        return { status: 'not-found' };
      if (scope.kind === 'current-answer' && !input.readExactAnswerBasis)
        return { status: 'unavailable' };
      ...
      if (
        ...
        if (
          ...
          cursor.version !== SESSION_INVENTORY_V1 ||
