...
export async function ensureChatConversation(options: {
  ...
  title?: string;
  /**
   * S1 of #1302: the project the turn was sent from, if any. Stamped into
...
// station#1293: how far back `isUserTurnAlreadyPersisted` scans looking for
...
/**
 * station#1293: true when `targetText` is already present as a user message
 ...
 * data loss" alternative #797's review explicitly rejected: crossing either
 ...
 *
 * station#1293 review (HIGH-1): the marker half of this rule is required,
 ...
 * literal tail (station#1293) — the original tail-only check
 ...
 * copy, creating the exact duplicate-bubble race #1293 reported. The bounded
 ...
 * after the #1293 review's HIGH-1 finding) keeps it from reaching back into
 ...
 *
 * Known residual race, accepted rather than papered over (#797 review): the
 * check is a read-then-write, and the framework's own user-turn save is queued
 * on a background queue rather than awaited. Since #914 this applies to every
