 ...
 * the spawn case launches the fake binary through the production
 * `MuseAdapter` spawn.
 */
 ...
 */
function assertNoHostMuseOutsideHome(): void {
  const hostDirs = [
    '/usr/bin',
    '/bin',
...
describe('native-engine adoption resolves CLIs like the engine spawn (#2663)', () => {
  it('adopts a muse found only in ~/.local/bin, and the engine spawn executes that absolute path', async () => {
    ...
    const binary = writeFakeMuse(join(host.home, '.local', 'bin'), host.marker);
...
describe('the login-shell PATH capture adoption now awaits (#2663)', () => {
