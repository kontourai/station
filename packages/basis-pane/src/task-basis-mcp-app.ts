...
// Re-grounded 2026-09-11 (Nightly 34573339929): the resource measured
// 572,702 bytes against the previous 512,000 ceiling, grown by legitimate
// feature work riding the window (#1562 honest basis/diff panes, the
// @kontourai/surface 3.2.0 bump in #1741) — not by an accidental dependency.
// The previous ceiling's failure mode was total: the throw happens while the
// station-control server REGISTERS its tools, so every MCP initialize
// returned 500 and the whole control surface was down, not just over budget.
// 640 KiB carries ~12% headroom over the measured size; the runtime guard is
// the last line — the tracked budget test in package-boundary.test.ts is
// where tightening happens.
