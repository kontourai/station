...
/**
 * Candidate identity proven by a provider-owned, still-pending enrollment
 * session. This is deliberately not a DeploymentAuthenticationResult and
 * carries no application continuation or authorization.
 */
export interface PendingEnrollmentSession {
  enrollmentId: string;
  sessionId: string;
  subject: string;
  displayName: string;
...
export type PendingEnrollmentSessionResult =
  | { kind: 'pending'; session: PendingEnrollmentSession }
  | {
      kind: 'invalid';
      reason:
        | 'invalid-credential'
...
export interface DeploymentAuthenticationProvider
  ...
  sessionReferences?: {
    ...
    /**
     * Provider-private enrollment sessions are candidate identity only. A
     * still-pending session must fail ordinary verify/authenticate checks.
     */
    pendingEnrollment?: {
      /**
       * Create a provider session tagged for one server-owned enrollment
       * attempt. The Station-generated identifier must be high entropy and
       * globally unique; Station must never reuse it.
       */
      create(
        /** Opaque, high-entropy identifier generated by trusted Station code. */
        enrollmentId: string,
        request: Request,
      ...
      /** Clear only the matching marker after continuation issuance is durable, before response delivery. */
      promote(
        enrollmentId: string,
        sessionId: string,
        signal: AbortSignal,
      ): Promise<void>;
      /** Remove only pending sessions for this attempt, optionally narrowed to one session. */
      discard(
        enrollmentId: string,
