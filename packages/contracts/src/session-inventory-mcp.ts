...
export function parseStationSessionInventoryMcpEnvelope(
  ...
  try {
    ...
    if (
      record.version !== STATION_SESSION_INVENTORY_MCP_VERSION ||
      Object.keys(record).length !== 2 ||
      !Object.prototype.hasOwnProperty.call(record, 'projection')
    )
      return null;
    const projection = parseSessionInventoryProjection(record.projection);
    if (
      !projection ||
