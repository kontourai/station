...
// inside vitest: it is spawned as its own standalone process so it can be
// killed by an external SIGTERM with no chance to run any afterEach --
// exactly the station#1812 failure mode (a corpus-coordinator timeout/abort
// killing the whole worker, or a plain Ctrl-C). Node 24 runs plain-typed
