...
const VERBS: Record<string, VerbSpec> = {
  ...
  delegate: {
    ...
    detail: [
      ...
      'never dispatches, interrupts, restarts, or answers anything — the',
      'delegated task keeps running. --timeout=<seconds> bounds the whole',
      ...
      'needs-action, wait-timeout (still running), observation-lost',
      '(transport lost — not a task failure), unknown, and interrupted,',
      ...
      'deadline expired while the task is still running (not a failure),',
