...
const VERBS: Record<string, VerbSpec> = {
  ...
  delegate: {
    ...
    usage: [
      ...
      'station delegate wait <task-id> [--on=<environment>] [--timeout=<seconds>] [--interval=<seconds>]',
    ...
    detail: [
      ...
      '',
      'wait (#2264) polls status until an honest outcome or the wait budget',
      'expires. OBSERVATION ONLY: a deadline, Ctrl-C, or a polling failure',
      'never dispatches, interrupts, restarts, or answers anything — waiting',
      'never stops the delegated task. --timeout=<seconds> bounds the whole',
      'wait (default 3600, max 86400); --interval=<seconds> spaces the polls',
      '(default 5, max 3600); each status read is bounded by the remaining',
      'budget. --timeout=0 or a malformed value is a usage error. Under',
      '--json, exactly one envelope is printed: ok is true only when the',
      'task completed; data.outcome distinguishes completed, failed,',
      'needs-action, wait-timeout (deadline, last observed active),',
      'observation-lost (not a task failure), unknown, and interrupted,',
      'alongside the observed conversationId/currentSessionId.',
      '',
      ...
      'failure (Station unreachable/timed out; also wait observation lost —',
      'not a task failure), 3 delegation rejection (bad target, not ready,',
      'or a deps-unavailable response — including an unsupported modelOptions',
      'key), 4 --on-request=fail found a request already pending right after',
      'dispatch, or wait observed needs_input/review_pending/blocked/a',
      'pendingRequest (session/task left alive). wait additionally: 5 wait',
      'deadline expired with the task last observed active (not a failure),',
      '6 task status unknown/unclassifiable, 130 interrupted by Ctrl-C',
      '(task unaffected).',
    ],
  },
  projects: {
