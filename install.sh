...
# second download to learn whom to trust.
# BEGIN GENERATED PINNED MANIFEST SIGNING KEYS
# Generated by scripts/install-script-generated.mjs from config/release-manifest-keys.json
...
# Per-channel install constants (ring <-> runtime mapping, ports, launcher).
# BEGIN GENERATED CHANNEL CONSTANTS
# Generated by scripts/install-script-generated.mjs from config/channel-ports.json.
...
INSTALLABLE_RUNTIME_CHANNELS='stable beta nightly'
RELEASE_RINGS_JSON='{"stable":{"runtimeChannel":"stable","prerelease":false},"preview":{"runtimeChannel":"beta","prerelease":true},"nightly":{"runtimeChannel":"nightly","prerelease":true}}'
...
# END GENERATED CHANNEL CONSTANTS
...
# `preview` is a ring, not a runtime; it was once accepted as a runtime name.
if [ "$requested_runtime_channel" = preview ]; then
...
channel_constants "$requested_runtime_channel" || \
  fail "STATION_CHANNEL must be one of: $INSTALLABLE_RUNTIME_CHANNELS"
...
configure_runtime_paths() {
  runtime_channel="$1"
  channel_constants "$runtime_channel" || \
...
if [ "$version" != latest ]; then
  # An exact pin names its ring: vX.Y.Z-<ring>.N for a prerelease ring,
  # vX.Y.Z for the unlabelled one. Every ring accepts exact pins, since a
  # rollback (STATION_INSTALL_ALLOW_ROLLBACK=1) needs an exact target.
  version_release_channel="$(node -e '
    const [version, ringsJson] = process.argv.slice(1);
    const rings = JSON.parse(ringsJson);
    const match = /^v(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-([a-z]+)\.[1-9][0-9]*)?$/.exec(version);
  ...
  ' "$version" "$RELEASE_RINGS_JSON")" || fail "invalid STATION_VERSION: $version"
...
if [ -n "$public_manifest_url" ]; then
  ...
  node -e '
    ...
    const [manifestFile,pinnedKeysJson,testKeyFile,valuesDir,ringsJson]=process.argv.slice(1);
    ...
    const rings=JSON.parse(ringsJson);
    const shapes=Object.fromEntries(Object.entries(rings).map(([ring,entry])=>[ring,new RegExp(`^${release}${entry.prerelease?`-${ring}\\.([1-9][0-9]*)`:""}$`)]));
    // Schema 1 carries a macOS artifact beside the portable one and is a
    // frozen, historical format: stable and preview only. Schema 2 is
    // portable-only and covers every installable ring.
    const artifactKeys=p.schemaVersion===1?["macos","portable"]:["portable"];
    const channels=p.schemaVersion===1?["stable","preview"]:Object.keys(rings);
  ...
  ' "$manifest_file" "$PINNED_MANIFEST_SIGNING_KEYS" "$test_key_file" "$manifest_values" "$RELEASE_RINGS_JSON" || {
    manifest_status=$?
...
candidate_identity="$(node -e '
  const fs = require("node:fs");
  const [path, sha, ref, releaseChannel, ringsJson] = process.argv.slice(1);
  const value = JSON.parse(fs.readFileSync(path, "utf8"));
  const rings = JSON.parse(ringsJson);
  const ring = typeof value?.releaseChannel === "string" && Object.hasOwn(rings, value.releaseChannel)
    ? rings[value.releaseChannel] : undefined;
  let runtimeChannel;
  let provenanceReleaseChannel;
  if (value?.schemaVersion === 2 && ring &&
...
' "$candidate/.station-release.json" "$release_sha" "$release_tag" "$release_channel" "$RELEASE_RINGS_JSON")" || fail 'release provenance is invalid'
...
if [ -e "$state_file" ] || [ -L "$state_file" ]; then
  ...
  node -e '
    ...
    const rings = JSON.parse(ringsJson);
    if (v?.schemaVersion !== 3 || typeof v.releaseChannel !== "string" || !Object.hasOwn(rings, v.releaseChannel) || v.channel !== rings[v.releaseChannel].runtimeChannel || typeof v.installRoot !== "string" || typeof v.stationHome !== "string" || typeof v.stationRoot !== "string") process.exit(1);
...
check_public_manifest_version() {
  ...
  version_order="$(node -e '
    const fs = require("node:fs");
    const [provenancePath, candidateTag, ringsJson] = process.argv.slice(1);
    const rings = JSON.parse(ringsJson);
    const parse = (tag) => {
      const match = /^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)(?:-([a-z]+)\.([1-9][0-9]*))?$/.exec(tag);
      if (!match || (match[4] !== undefined && !(Object.hasOwn(rings, match[4]) && rings[match[4]].prerelease))) return null;
  ...
  ' "$current_link/.station-release.json" "$release_tag" "$RELEASE_RINGS_JSON")" || {
